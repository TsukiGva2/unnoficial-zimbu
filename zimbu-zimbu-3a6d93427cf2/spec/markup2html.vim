...
func! Process(text)
  ...
  let text = substitute(text, ' \*\([0-9A-Za-z_-]\+\)\* ', ' <b>\1</b> ', 'g')

  if text =~ '\[\[[^]]*\]\]'
    let text = substitute(text, '\[\[\([^]]*\)\]\]', '\=MakeLink(submatch(1))', 'g')
  endif
  if text =~ '$$DOCS'
    let text = substitute(text, '$$DOCS', 'http://www.moolenaar.net/z/zimbu/docs', 'g')
  endif
  return text
endfunc
...
while l <= line('$')
  ...
  if type != ''
    ...
    if l > 2
      if type == 'h1'
	call append(l - 1, '<div class="sep1"></div>')
      else
	call append(l - 1, '<div class="sep2"></div>')
      endif
      let l = l + 1
    endif
    if text != 'Contents' && text != 'Zimbu Language Specification' && text != 'Zimbu Templates Specification'
  ...
  if type != ''
    call EndPara()
    let titletext = substitute(text, '^##* ', '', '')
    let text = name
    if onclick != ''
      let text .= '<div' . onclick . '>'
    endif
    let text .= '<' . type . '>' . HtmlEscape(titletext) . '</' . type . '>'
    if onclick != ''
  ...
  if text =~ '^<command'
    " Block with a command line command, turn into <pre>
    let start = l
    let l = search('</command', 'W')
    if l == 0
      break
    endif
    call setline(start, '<pre class="command">')
    call setline(l, '</pre>')
    let l = l + 1
    continue
  endif
...
" For ZUT replace the title.
if expand('%:r') =~ 'zut'
  %s/Zimbu Language Specification/Zimbu Templates Specification/
endif
