...
COMP_SOURCES = \
	...
	genJava/WriteArrayJava.zu \
	genJava/WriteDictJava.zu \
	genJava/WriteJava.zu \
	genJava/WriteListJava.zu \
	lib/BYTELoader.zu \
	lib/BYTEModule.zu \
	...
	lib/ZUTLoader.zu \
	lib/ZWTLoader.zu \
	parser/FilePosition.zu \
	parser/Input.zu \
	parser/Parser.zu \
	parser/Token.zu \
	parser/TokenInput.zu \
	parser/Tokenizer.zu \
	parser/Visibility.zu \
	parser/zui.proto \
	zimbu2c.zu
...
PLUGINZUT_SRC = \
	plugin/zut/ZutParser.zu \
	plugin/zut/ZutTokenizer.zu \
	plugin/zut/ZutTokenType.zu \
	parser/Parser.zu \
	parser/Token.zu \
	parser/TokenInput.zu \
	parser/Tokenizer.zu \
...
# Uncomment to build with unused symbols generated. For debugging.
...
#TESTJE_ARGS = --manage=linked --exitclean --reportErrorInUnused
...
zimbu_test$(EXE): $(SHARED_SOURCES) $(MAIN_SOURCES) $(INPUT_FILES)
	-$(WRITABLE) zimbu_test.zu
	$(COPY) zimbu.zu zimbu_test.zu
	$(READONLY) zimbu_test.zu
...
# Push the current commit to Bitbucket.
...
bootstrap:
	...
	cd thirdparty/pcre; $(COMPILER) $(CCARG) -c pcrelib.c
...
# Build ALL targets that should build, except for the "test" target.  Used to
# verify compiler changes.
# Requires "nodejs" for the jstest target.
...
# Like "testrun", but only where the source file was updated.
testrunupdate: testrunner
	./testrunner --update '$(CURDIR)zimbu_test $(THREAD_ARGS) $(COMPILER_ARGS)'

testrunner: testrunner.zu
...
# Small program that filters the jstest output for things that are different
# but OK.  Mainly because int is stored in a float.
fixjstestout: fixjstestout.zu
	$(CURDIR)zimbu_test -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) fixjstestout.zu
...
test_testprog: build_zimbu_test fixtestout test.ok
	@echo
...
test_zimbu: build_zimbu_test
	$(CURDIR)zimbu_test -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) test testZimbu.zu

test_zimbu_js: build_zimbu_test
	@echo
	$(CURDIR)zimbu_test --js -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) --interpreter $(NODEJS) test testZimbu.zu

# TODO: use testZimbu.zu instead of the copied testZimbuJava.zu
test_zimbu_java: build_zimbu_test
	@echo
	$(CURDIR)zimbu_test --java -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) --interpreter $(JAVA) test testZimbuJava.zu

# Test that the new version can build itself
zimbu_test_test$(EXE): build_zimbu_test
	@echo
	-$(WRITABLE) zimbu_test_test.zu
	$(COPY) zimbu.zu zimbu_test_test.zu
	$(READONLY) zimbu_test_test.zu
	$(CURDIR)zimbu_test -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) zimbu_test_test.zu
	-$(WRITABLE) zimbu_test_test2c.zu
	$(COPY) zimbu2c.zu zimbu_test_test2c.zu
	$(READONLY) zimbu_test_test2c.zu
...
test_test_testprog: zimbu_test_test$(EXE) test.ok fixtestout
	@echo
...
# Test that the new version can build and run the Javascript test program.
# Requires the nodejs program.
jstest: build_zimbu_test jstest.ok fixjstestout
	@echo
	$(CURDIR)zimbu_test --js -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) jstestprog.zu
	@echo
	$(NODEJS) jstestprog.js argone -x argtwo -- -a > jstest.out
	$(CURDIR)fixjstestout jstest.out
	$(DIFF) jstest.ok jstest.out

jstest.ok: testdir/Common_start.ok jstest_start.ok testdir/Common.ok
	$(CAT) $+ >jstest.ok
...
# This is very slow!
profile:
	-$(WRITABLE) zimbu_test2c.zu
	$(COPY) zimbu2c.zu zimbu_test2c.zu
	$(READONLY) zimbu_test2c.zu
...
# Build zimbu_test for profiling allocations, then run it to build
# zimbu_test_test2c.
profalloc:
	-$(WRITABLE) zimbu_test2c.zu
	$(COPY) zimbu2c.zu zimbu_test2c.zu
	$(READONLY) zimbu_test2c.zu
	$(CURDIR)zimbu -v --profalloc $(BUILD_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) zimbu_test2c.zu
	-$(WRITABLE) zimbu_test_test2c.zu
	$(COPY) zimbu2c.zu zimbu_test_test2c.zu
	$(READONLY) zimbu_test_test2c.zu
	$(CURDIR)zimbu_test -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) zimbu_test_test2c.zu
...
zudocu: zudocu/zudocu

zudocu/zudocu: $(DOCZUFILES) build_zimbu_test
	$(CURDIR)zimbu_test $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) -v zudocu/zudocu.zu

docs: $(DOCINPUT) zudocu/zudocu
	$(RMCMD) docs/*.html
	zudocu/zudocu $(DOCINPUT) docs
...
picedit: doesnotexist
	cd picedit; make $(MAKEFILEARG)
...
sleepsort: sleepsort.zu build_zimbu_test
	$(CURDIR)zimbu_test $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) sleepsort.zu
...
proftestje: testje.zu build_zimbu_test
	$(CURDIR)zimbu_test -v --profalloc $(TESTJE_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) testje.zu -o proftestje$(EXE)

testje2: testje.zu zimbu_test_test$(EXE)
	$(CURDIR)zimbu_test_test -v $(TESTJE_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) testje.zu

jstestje: testje.zu build_zimbu_test
	$(CURDIR)zimbu_test --js -v $(TESTJE_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) testje.zu

javatestje: testje.zu build_zimbu_test
	$(CURDIR)zimbu_test --java -v $(TESTJE_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) testje.zu
...
clean:
	...
	$(RMCMD) zudocu/zudocu zudocu/ZUDIR/*
	$(RMCMD) zwtdemo/zwtdemo zwtdemo/ZUDIR/*
	$(RMCMD) zutdemo/zutdemo zutdemo/ZUDIR/*
	$(RMCMD) picedit/picedit picedit/ZUDIR/*
...
runjstestje: jstestje
	$(NODEJS) testje.js

runjavatestje: javatestje
	$(JAVA) -cp testje.jar testje
...
# without leaks.
test_testprog_valgrind: build_zimbu_test test.ok
	@echo
	$(CURDIR)zimbu_test -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) testprog.zu
	@echo
...
valgrind_zimbu_test: build_zimbu_test test.ok
	@echo
...
test_test_testprog_valgrind: zimbu_test_test$(EXE) test.ok
	@echo
...
test.ok: testdir/Common_start.ok test_start.ok testdir/Common.ok
	$(CAT) $+ >test.ok
...
pluginproto$(EXE): $(PLUGINPROTO_SRC) build_zimbu_test
	...
	$(COPY) plugin/proto/ZUDIR/pluginproto.c plugin/proto/pluginproto.c
...
pluginzut$(EXE): $(PLUGINZUT_SRC)
	...
	$(COPY) plugin/zut/ZUDIR/pluginzut.c plugin/zut/pluginzut.c

# Tests for pluginzut
pluginzuttest: pluginzut$(EXE)
	$(CURDIR)zimbu_test -v $(TEST_ARGS) $(KEEP_UNUSED) $(COMPILER_ARGS) -o testdir/ZUDIR/zutTest$(EXE) testdir/zutMain.zu
