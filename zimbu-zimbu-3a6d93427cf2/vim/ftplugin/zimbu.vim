...
" Also recognize Node.js errors: "fileName:lineNr"
setlocal errorformat^=ERROR:\ %f\ line\ %l\ col\ %c:\ %m,%f\ line\ %l\ col\ %c:\ %m,ERROR:\ %m,%f:%l
" and Node.js stack backtrace: "at Zthrow (/path/zimbu/jstestprog.js:618:8)"
setlocal errorformat^=%m(%f:%l:%c)

" When the matchit plugin is loaded, this makes the % command skip parens and
" braces in comments.
let b:match_words = '\%(^\s*\)\@<=\%(MODULE\|CLASS\|INTERFACE\|BITS\|ENUM\|SHARED\|PIECE\|INCLUDE\|TEMPLATE\|CSS\|IF\|IFNIL\|GENERATE_IF\|WHILE\|REPEAT\|WITH\|DO\|FOR\|SWITCH\|TRY\|FUNC\|PROC\)\>\|\%(^\s*\)\@<=NEW(.*)\%(\s\+@[a-z=]\+\)*\s*$\|{\s*$:\%(^\s*\)\@<=\%(ELSE\|ELSEIF\|GENERATE_ELSE\|GENERATE_ELSEIF\|CATCH\|FINALLY\)\>:\%(^\s*\)\@<=\%(}\|\<UNTIL\>\)'
...
" Disabled, typing "arg" is more frequent than "ARG".
...
iabbr <buffer> <expr> piece GCUpperSpace("piece")
...
iabbr <buffer> <expr> augments GCUpper("augments")
iabbr <buffer> <expr> grows GCUpper("grows")
...
iabbr <buffer> <expr> template GCUpperSpace("template")
...
func! ZimbuGoStartBlock()
  ...
  while line('.') > 1
    let l = getline('.')
    if l =~ '^\s*\(FUNC\|PROC\|MAIN\|ENUM\|CLASS\|INTERFACE\|TEMPLATE\)\>'
      let depth -= 1
      if depth < 0
	if line('.') != start
	  break
    ...
    elseif l =~ '^\s*\(IF\|SWITCH\|FOR\|WHILE\)\>'
      if depth > 0
	let depth -= 1
      endif
    elseif l =~ '^\s*}'
...
func! ZimbuGoEndBlock()
  /^\s*\(FUNC\|PROC\|MAIN\|ENUM\|CLASS\|INTERFACE\|TEMPLATE\)\>
endfunc
